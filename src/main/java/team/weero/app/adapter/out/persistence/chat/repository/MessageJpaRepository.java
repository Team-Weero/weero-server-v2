...
public interface MessageJpaRepository extends JpaRepository<MessageJpaEntity, UUID> {
  ...
  List<MessageJpaEntity> findUnreadMessagesByChatRoom(
      @Param("chatRoom") ChatRoomJpaEntity chatRoom);

  @Query(
      "SELECT COUNT(m) FROM MessageJpaEntity m WHERE m.chatRoom.id = :chatRoomId AND m.sender.id != :userId AND m.readStatus = false")
  int countUnreadMessagesByChatRoomIdAndReceiverId(
      @Param("chatRoomId") UUID chatRoomId, @Param("userId") UUID userId);
}
