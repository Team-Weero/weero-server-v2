...
public class ConcernPersistenceAdapter implements ConcernPort {
  ...
  public ConcernPersistenceAdapter(
      ConcernJpaRepository concernJpaRepository, ConcernMapper concernMapper) {
  ...
  public Optional<Concern> findById(UUID id) {
    return concernJpaRepository.findById(id).map(concernMapper::toDomain);
  }

  @Override
  public List<Concern> findByStudentId(UUID studentId) {
    return concernJpaRepository.findByStudentIdOrderByCreatedAtDesc(studentId).stream()
  ...
  public List<Concern> findByIsResolved(boolean isResolved) {
    return concernJpaRepository.findByIsResolvedOrderByCreatedAtDesc(isResolved).stream()
        .map(concernMapper::toDomain)
        .toList();
  }
  ...
  public List<Concern> findAll() {
    return concernJpaRepository.findAllByOrderByCreatedAtDesc().stream()
        .map(concernMapper::toDomain)
        .toList();
  }
