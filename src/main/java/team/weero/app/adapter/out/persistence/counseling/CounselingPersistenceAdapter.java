...
public class CounselingPersistenceAdapter implements CounselingPort {
  ...
  public Optional<CounselingApplication> findById(UUID id) {
    return counselingJpaRepository.findById(id).map(counselingMapper::toDomain);
  }
  ...
  @Override
  public boolean existsByTeacherIdAndCounselDateAndTime(
      UUID teacherId, LocalDate counselDate, LocalTime time) {
    return counselingJpaRepository.existsByTeacherIdAndCounselDateAndTime(
        teacherId, counselDate, time);
  }
