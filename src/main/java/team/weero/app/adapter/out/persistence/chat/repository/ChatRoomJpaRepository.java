...
public interface ChatRoomJpaRepository extends JpaRepository<ChatRoomJpaEntity, UUID> {
  ...
  @Query(
      "SELECT c FROM ChatRoomJpaEntity c WHERE c.startUser.id = :userId OR c.receiverUser.id = :userId")
  List<ChatRoomJpaEntity> findByUserId(@Param("userId") UUID userId);

  @Query(
      "SELECT c FROM ChatRoomJpaEntity c WHERE "
  ...
  Optional<ChatRoomJpaEntity> findByParticipants(
      @Param("user1") UserJpaEntity user1, @Param("user2") UserJpaEntity user2);
}
