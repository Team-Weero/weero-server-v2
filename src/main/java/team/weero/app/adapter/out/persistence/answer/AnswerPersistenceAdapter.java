...
public class AnswerPersistenceAdapter implements AnswerPort {
  ...
  public AnswerPersistenceAdapter(
      AnswerJpaRepository answerJpaRepository,
      AnswerMapper answerMapper,
      ConcernJpaRepository concernRepository,
  ...
  public Optional<Answer> findById(UUID id) {
    return answerJpaRepository.findById(id).map(answerMapper::toDomain);
  }

  @Override
  public List<Answer> findByConcernId(UUID concernId) {
    ConcernJpaEntity concern =
        concernRepository
            .findById(concernId)
