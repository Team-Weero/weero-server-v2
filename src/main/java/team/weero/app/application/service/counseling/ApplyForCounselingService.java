...
public class ApplyForCounselingService implements ApplyForCounselingUseCase {
  ...
  public void execute(CounselingRequest request) {
    studentPort.findById(request.studentId()).orElseThrow(NotFoundException::new);

    teacherPort.findById(request.teacherId()).orElseThrow(NotFoundException::new);

    boolean exists =
        counselingPort.existsByTeacherIdAndCounselDateAndTime(
            request.teacherId(), request.date(), request.time());
    if (exists) {
      throw new DuplicateReservationException();
    }

    CounselingApplication counseling =
        CounselingApplication.create(
            request.studentId(),
            request.teacherId(),
            request.time(),
            request.date(),
